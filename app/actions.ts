  ...
  if (isDrizzleConfigured() && db) {
    try {
      await db.insert(schema.products).values({
        ...
        imageUrl: product.imageUrl || (product.images && product.images[0]) || 'https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&w=1000&q=85',
  ...
  if (isDrizzleConfigured() && db) {
    try {
      await db.insert(schema.orders).values({
        ...
        porterTrackingId: order.porterTrackingId || `TRK-DSP-${order.orderNumber || order.id || Date.now()}`,
      ...
      if (rows && rows.length > 0) {
        const formatted: Order[] = rows.map((r: any) => {
          ...
          return {
            ...
            discount: (r as any).discount ? Number((r as any).discount) : 0,
